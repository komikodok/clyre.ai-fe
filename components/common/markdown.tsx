...
function Markdown({ textContent }: { textContent: string }) {
  return (
    <ReactMarkdown
      remarkPlugins={[remarkGfm]}
      components={{
        h1: ({ ...props }) => (
          <h1 {...props} className="text-2xl fon2-bold my-1" />
        ),
        h2: ({ ...props }) => (
          <h2 {...props} className="text-xl font-bold my-1" />
        ),
        h3: ({ ...props }) => (
        ...
        table: ({ ...props }) => (
          <div className="overflow-x-auto my-4">
            <table
              {...props}
              className="w-full border-collapse border border-slate-100/20 rounded-md text-sm"
            />
          </div>
        ),
        thead: ({ ...props }) => (
        ...
        tbody: ({ ...props }) => (
          <tbody {...props} className="divide-y divide-slate-100/10" />
        ),
        tr: ({ ...props }) => (
          <tr {...props} className="hover:bg-slate-100/5 transition-colors" />
        ),
        th: ({ ...props }) => (
          <th
            {...props}
            className="px-4 py-2 text-left font-semibold text-slate-200 border-b border-slate-100/20"
          />
        ),
        td: ({ ...props }) => (
          <td
            {...props}
            className="px-4 py-2 text-slate-300 border-r border-slate-100/10 last:border-r-0"
          />
        ),
        pre: ({ node, ...props }) => {
              ...
              <div className="flex justify-between items-center">
                ...
                <Tooltip content="Copy" className="text-[10px]">
                  <Button
                    ...
                    className="cursor-pointer w-5 h-5 active:scale-90 transition-transform !bg-transparent"
                  >
                    <Copy className="stroke-white/50 size-2 md:size-3" />
                  </Button>
                </Tooltip>
              </div>
              <div className="px-2 text-xs">{props.children}</div>
            </pre>
          );
        },
      }}
